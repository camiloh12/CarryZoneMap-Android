...
object UsOverlayGenerator {
    ...
    fun generateAccurateNonUsOverlay(context: Context): Feature {
        val overlayFeature = tryGenerateAccurateOverlay(context)
        return overlayFeature ?: run {
            Timber.w("Falling back to simplified overlay")
            generateSimplifiedNonUsOverlay()
        }
    }

    /**
     * Attempts to generate an accurate overlay from GeoJSON data.
     * Returns null if the overlay cannot be generated.
     */
    private fun tryGenerateAccurateOverlay(context: Context): Feature? {
        return try {
            // Load US boundary from assets
            Timber.d("Loading US boundary GeoJSON from assets...")
            val usBoundaryFeature =
                GeoJsonLoader.loadFirstFeatureFromAssets(
                    context,
            ...
            if (geometry !is MultiPolygon) {
                Timber.w("US boundary is not a MultiPolygon")
                return null
            }
            ...
            Feature.fromGeometry(overlayPolygon)
        } catch (e: Exception) {
            Timber.e(e, "Failed to generate accurate overlay")
            null
        }
    }
    ...
    fun generateNonUsOverlay(): Feature {
        ...
        val stateHoles =
            UsBoundaryValidator.US_STATE_BOUNDING_BOXES.map { bbox ->
                listOf(
