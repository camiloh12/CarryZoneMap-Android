...
import org.maplibre.android.style.expressions.Expression.* // Required for match, literal, get, stop
...
class FeatureLayerManager {
    ...
    fun addSourceAndLayer(style: Style, initialFeatures: List<Feature>) {
        val source = GeoJsonSource(USER_PINS_SOURCE_ID, FeatureCollection.fromFeatures(initialFeatures.toTypedArray()))
        style.addSource(source)

        val layer = CircleLayer(USER_PINS_LAYER_ID, USER_PINS_SOURCE_ID).withProperties(
    ...
    fun updateDataSource(style: Style, currentFeatures: List<Feature>) {
